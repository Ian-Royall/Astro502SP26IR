...
# === Check old grid (before NIR) ===
print("Checking old grid (parsec_grid.npz)")
data_old = np.load('parsec_grid.npz')
grid_old = {k: data_old[k] for k in data_old if k not in ['masses', 'logages', 'fehs']}

for band in grid_old:
    nan_count = np.sum(np.isnan(grid_old[band]))
    total = grid_old[band].size
    percent_nan = 100 * nan_count / total
    print(f"{band}: {nan_count:,} NaNs / {total:,} points = {percent_nan:.1f}% missing")

# === Check new grid with NIR ===
print("\nChecking new grid (parsec_grid_with_NIR.npz)")
data_new = np.load('parsec_grid_with_NIR.npz')
grid_new = {k: data_new[k] for k in data_new if k not in ['masses', 'logages', 'fehs']}

for band in grid_new:
    nan_count = np.sum(np.isnan(grid_new[band]))
    total = grid_new[band].size
    percent_nan = 100 * nan_count / total
    print(f"{band}: {nan_count:,} NaNs / {total:,} points = {percent_nan:.1f}% missing")